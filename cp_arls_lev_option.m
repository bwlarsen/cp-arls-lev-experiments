function [P,Uinit,output] = cp_arls_lev_option(X,R,varargin)
...
% - This version is NOT meant for the main toolbox; it simply allows
% comparison to sampling probabilities we know from theory are sub-optimal
...
params.addParameter('sampling', 'uniform', @(x) ismember(x,{'leverage', 'uniform', 'lengthsqr'}));
...
%% Set up cell for sampling method:
% This will only
Ulev = cell(d,1);

if strcmp(sampling, 'leverage')  
    for kk = 1:d
        Ulev{kk} = tt_leverage_scores(U{kk});
    end
elseif strcmp(sampling, 'uniform')
    for kk = 1:d
        Ulev{kk} = ones(sz(kk), 1);
    end
elseif strcmp(sampling, 'lengthsqr')
    for kk = 1:d
        Ulev{kk} = sum(U{kk}.^2, 2);
    end
else
    error('The selected sampling method is not supported');
end
...
for epoch = 1:maxepochs
    ...
    for eiters = 1:epochsize
        ...
        for k = dimorder(1:end)
            ...
            [Unew, ~] = tt_sampled_solve(X, U, Ulev, k, nsamplsq, [], thresh, Xfidxs{k},0,true);
            ...
            % Update the sampling probabilities of updated factor
            U{k} = Unew;
            
            if strcmp(sampling, 'leverage')  
                Ulev{k} = tt_leverage_scores(U{k});
            elseif strcmp(sampling, 'uniform')
                %Ulev{k} = ones(sz{k}, 1);
            elseif strcmp(sampling, 'lengthsqr')           
                Ulev{k} = sum(U{k}.^2, 2);
