%% Results file
resultfile = 'arls_runs_enron-results';
resultfile_als = 'als_runs_enron-results';
...
methodname = cell(2,1);
for kk = 1:2
    methodname{kk} = sprintf('RRF s = 2^{%d}', log2(srng(kk)));
end
methodname{3} = 'Not Random';
for kk = 1:2
    methodname{kk+3} = sprintf('Random Guassian s = 2^{%d}', log2(srng(kk)));
end
...
[itime, ifits, isumm, ifitsext] = interpl_plus(traces, 2.0);
...
fid = fopen('arls_runs_enron_diary.txt','r');
str = fgetl(fid);
...
% Open file
fid = fopen('../cprand-sparse-paper/data-enron-fittrace-raw.csv','w');
...
% Open file
fid = fopen('../cprand-sparse-paper/data-enron-fittrace-interpl.csv','w');
...
% Open file
fid = fopen('../cprand-sparse-paper/data-enron-epochtimes.csv','w');
...
% Open file
fid = fopen('../cprand-sparse-paper/data-enron-finalfits.csv','w');
...
print('../cprand-sparse-paper/fig-enron-runs','-dpng','-r300');
...
xtl = {'s^{18} Random Init', 's^{18} RRF', 's^{19} Random Init', 's^{19} RRF', 'Standard'};
...
print('../cprand-sparse-paper/fig-enron-fits','-dpng','-r300');
...
print('../cprand-sparse-paper/fig-enron-totaltimes','-dpng','-r300');
