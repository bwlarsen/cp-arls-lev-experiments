...
% truefit specifies whether or not the final fit is calculated
% Code currently does not have option
...
% This is the old code for calculating the fit; eventually eliminate
...
%     fit = 1 -  normDiff / normX;
    ...
    if (mod(epoch,printitn)==0) || ((printitn>0) && (flag==0))
        fprintf('Iter %2dx%d: f~ = %e time = %.1fs (fit time = %.1fs) newi = %i\n', epoch, epochsize, fit, time_trace(epoch+1) - time_trace(epoch), fit_time, newi);
    end
